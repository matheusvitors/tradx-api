...
describe.skip('Import operacoes', () => {
	...
	const csvFile = path.resolve(__dirname,  '../', '../', 'assets', 'operacoes-teste.csv');
	...
	it('should return 404 if not send file', async () => {
		const response = await importOperacoesByCsvController({ operacaoRepository, ativoRepository, contaRepository, regraEntradaRepository, contaId: 'abc', file: ''});
		expect(response.status).toEqual(404);
	});

	it('should return 422 if a field is invalid from csv file', async () => {
		const invalidCsvFile = path.resolve(__dirname, '../', '../', 'assets', 'operacoes-teste-invalid.csv');
		const response = await importOperacoesByCsvController({ operacaoRepository, ativoRepository, contaRepository, regraEntradaRepository, contaId: 'abc', file: invalidCsvFile});
		expect(response.status).toEqual(422);
	});

	it('should return 422 if date field is invalid from csv file', async () => {
		const invalidCsvFile = path.resolve(__dirname,'../', '../', 'assets', 'operacoes-teste-invalid-date.csv');
		const response = await importOperacoesByCsvController({ operacaoRepository, ativoRepository, contaRepository, regraEntradaRepository, contaId: 'abc', file: invalidCsvFile});
		expect(response.status).toEqual(422);
	...
	it('should return 422 if a ativo is invalid from csv file', async () => {
		const invalidCsvFile = path.resolve(__dirname, '../', '../', 'assets', 'operacoes-teste-invalid-ativo.csv');
		const response = await importOperacoesByCsvController({ operacaoRepository, ativoRepository, contaRepository, regraEntradaRepository, contaId: 'abc', file: invalidCsvFile});
		expect(response.status).toEqual(422);
	...
	it('should return 404 if a conta is invalid from csv file', async () => {
		const response = await importOperacoesByCsvController({ operacaoRepository, ativoRepository, contaRepository, regraEntradaRepository, contaId: '', file: csvFile});
	...
	it('should return 422 if a regra is invalid from csv file', async () => {
		const invalidRegraCsvFile = path.resolve(__dirname, '../', '../', 'assets', 'operacoes-teste-invalid-regra.csv');

		const response = await importOperacoesByCsvController({ operacaoRepository, ativoRepository, contaRepository, regraEntradaRepository, contaId: 'abc', file: invalidRegraCsvFile});
